...
(function(){
  ...
  function buildBG(){
    ...
    g.addColorStop(0,'#d8f2fd'); g.addColorStop(.28,'#a5ddf6'); g.addColorStop(.62,'#6ec4ea'); g.addColorStop(1,'#3ea3db');
    o.fillStyle=g; o.fillRect(0,0,cols,rows);
    // bubbles — refracted by ripples, like the ones in the banner artwork
    const bubbles=Math.round((cols*rows)/900);
    for(let i=0;i<bubbles;i++){
      const sxp=Math.random()*cols, syp=Math.random()*rows;
      const a=0.30+Math.random()*0.55, rad=Math.random()<0.14?1.6:0.9;
      const sg=o.createRadialGradient(sxp,syp,0,sxp,syp,rad*2.4);
      sg.addColorStop(0,'rgba(255,255,255,'+a+')'); sg.addColorStop(1,'rgba(255,255,255,0)');
      o.fillStyle=sg; o.fillRect(sxp-3,syp-3,6,6);
    }
    // sunlight halo + core (the light the ripples catch)
    mcx=cols*.5; mcy=rows*.13; const mr=Math.min(cols,rows)*.62;
    let mg=o.createRadialGradient(mcx,mcy,0,mcx,mcy,mr);
    mg.addColorStop(0,'rgba(255,255,255,.62)'); mg.addColorStop(.26,'rgba(214,240,252,.26)'); mg.addColorStop(1,'rgba(214,240,252,0)');
    o.fillStyle=mg; o.fillRect(0,0,cols,rows);
    let md=o.createRadialGradient(mcx,mcy,0,mcx,mcy,mr*.12);
    md.addColorStop(0,'rgba(255,255,255,.95)'); md.addColorStop(1,'rgba(255,255,255,0)');
  ...
  function drawOverlay(now){
    ...
    if(rings.length){
      ...
      for(let i=rings.length-1;i>=0;i--){
        ...
        ctx.strokeStyle='rgba(255,255,255,'+a+')'; ctx.lineWidth=2*(1-k)+0.4;
    ...
    for(const m of motes){
      ...
      g.addColorStop(.45,'rgba(206,238,252,'+(a*0.4)+')');
      g.addColorStop(1,'rgba(206,238,252,0)');
